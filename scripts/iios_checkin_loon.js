...
const Env = (() => {
  ...
  function write(key, value) {
    if (isQX) return $prefs.setValueForKey(String(value), key);
    if (isSurge || isLoon) return $persistentStore.write(String(value), key);
    return false;
  }
...
if (isRequestPhase()) {
  ...
  function handleFinal(obj) {
    ...
    if (obj.status === 'done' && obj.data) {
      ...
      if (obj.data.status === 'already_done') {
        return finish('iios.fun', '今日已签到', r.message || '今日已完成');
      }
      if (obj.data.status === 'checked_in') {
        return finish('iios.fun', '签到成功', r.message || '签到成功');
      }
    }
      ...
      .then(resp => {
        ...
        if (!obj || !obj.ok || !obj.jobId) {
          return finish(
            'iios.fun',
            '启动失败',
            obj.message || resp.body || 'start failed'
          );
        }
  ...
  if (!email || !password) {
    finish('iios.fun', '缺少账号参数', '请先在本地存储填写邮箱和密码');
  } else {
