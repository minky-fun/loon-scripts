...
class $ {
	/**
	 * 批量导入远程模块并挂载到工具类。
	 * @param {...Array} input - 模块导入配置列表
	 * @returns {Promise<Array>} 模块导入结果
	 */
	...
	/**
	 * 将 GitHub 链接转换为 jsDelivr CDN 链接。
	 * @param {string} path - 原始模块地址
	 * @returns {string} 转换后的模块地址
	 */
	static #cdn(path) {
	...
	/**
	 * 根据导入规则把模块导出挂载到工具类。
	 * @param {string|Function|Array|undefined} rule - 挂载规则
	 * @param {any} exp - 模块导出对象
	 * @param {string} expName - 导出名称
	 * @param {number} depth - 对象展开深度
	 * @returns {void}
	 */
